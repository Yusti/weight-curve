...
export default class WeightCurveScreen extends React.Component {
    ...
    return (
      <View style={styles.container}>
        <Header
          ...
          RightButton={this._renderAddButton}
        />
        <ScrollView>
          <View style={styles.curveContainer}>
            <View style={styles.curveField}>{this._renderCurvePoints()}</View>
            <View style={styles.curveFooter}>
              {this._renderDetailsCell('Start', `${weightData[0].weight} kg`)}
              {this._renderDetailsCell(
                'Change',
                `${(
                  weightData[weightDataLength - 1].weight - weightData[0].weight
                ).toFixed(1)} kg`
              )}
              {this._renderDetailsCell(
                'Current',
                `${weightData[weightDataLength - 1].weight} kg`
              )}
            </View>
          </View>
  ...
  _renderCurvePoints = () => {
    ...
    for (let i = 0; i < weightDataLength; i++) {
      ...
      if (i < weightDataLength - 1) {
        ...
        const yDiff =
          +new Date(weightData[i + 1].date) - +new Date(weightData[i].date)
    ...
    weightData.forEach((item, index) => {
      const bottom =
        maxWeight === minWeight ? 60 : (item.weight - minWeight) / weightDiff
      ...
      points.push(
        <View
          key={`point-${item.date}`}
          style={[styles.point, { bottom, left }]}
        />
      )
    })
    return [...points, ...lines]
  ...
  _setUserDueDate = () => {
    ...
    firebase
      ...
      .on('value', snapshot => {
        this.setState({
          dueDate: moment(snapshot.val().dueDate)
            .subtract(280, 'days')
            .format('YYYY-MM-DD'),
        })
      })
